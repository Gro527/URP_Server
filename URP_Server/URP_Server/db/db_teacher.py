...
def get_tea_by_id(id):
    with app.app_context():
        try:
            ...
            cursor.execute(
                "select tcc.course_id,c.name,tcc.class_id from teacher_course_class tcc join course c on tcc.course_id = c.id where teacher_id = '%s'" % (id))
            res = cursor.fetchall()
            for row in res:
                tc = {
                    'course_id': row[0],
...
def set_score(stu_id, course_id, score):
    with app.app_context():
        try:
            db = dbconnect.get_conn()
            cursor = db.cursor()
            cursor.execute("update student_course set score = %s where student_id = '%s' and course_id = '%s'" % (
